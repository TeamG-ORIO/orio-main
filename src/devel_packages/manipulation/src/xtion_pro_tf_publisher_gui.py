#!/usr/bin/env python
"""
Interactive calibration GUI for the Xtion Pro / RealSense static TF.
...
Buttons:
  Save to YAML      – overwrites realsense_tf.yaml
...
def compute_camera_link_pose(panda_to_camrgb_pose, tf_buffer):
    try:
        t = tf_buffer.lookup_transform(
            'camera_rgb_optical_frame', 'camera_link',
            rospy.Time(0), rospy.Duration(5.0))
        rgb_to_base = Pose()
        rgb_to_base.position.x = t.transform.translation.x
        rgb_to_base.position.y = t.transform.translation.y
        rgb_to_base.position.z = t.transform.translation.z
        rgb_to_base.orientation.x = t.transform.rotation.x
        rgb_to_base.orientation.y = t.transform.rotation.y
        rgb_to_base.orientation.z = t.transform.rotation.z
        rgb_to_base.orientation.w = t.transform.rotation.w
        M = np.matmul(pose_to_transformation_matrix(panda_to_camrgb_pose),
                      pose_to_transformation_matrix(rgb_to_base))
...
T_RANGE   = 2.0     # ± metres
T_STEPS   = 4000
Q_RANGE   = 1.0     # ± 1 for each quaternion component
Q_STEPS   = 2000
RPY_RANGE = 180.0   # ± degrees
RPY_STEPS = 3600


def _make_row(parent, row, label, slider_from, slider_to, slider_steps,
              slider_var, text_var, text_width=12, on_press=None, on_release=None):
    """One row: label | slider | text entry.  Returns (slider, entry)."""
    ttk.Label(parent, text=label, width=4, anchor="e").grid(
    ...
    if on_release:
        sl.bind("<ButtonRelease-1>", on_release)
...
class CalibrationGUI:
    def __init__(self, root, yaml_path, tf_buffer):
        ...
        # Guard to stop circular updates between quat ↔ rpy
        self._updating = False

        # Undo history: each entry is a dict snapshot of all values
        self._history = collections.deque(maxlen=50)
        self._snapshot_pending = False   # coalesce rapid slider moves

        root.title("Camera TF Calibration")
        ...
        # ---- Translation ------------------------------------------------
        tf = ttk.LabelFrame(root, text="Translation (metres)", padding=8)
        tf.grid(row=0, column=0, padx=10, pady=6, sticky="ew")

        self.t_sl  = {}   # axis -> DoubleVar used by slider
        self.t_txt = {}   # axis -> StringVar used by entry
        for i, (axis, val) in enumerate(zip(["x", "y", "z"], [tx, ty, tz])):
            sv = tk.DoubleVar(value=val)
            tv = tk.StringVar(value=f"{val:.5f}")
            self.t_sl[axis]  = sv
            self.t_txt[axis] = tv
            _make_row(tf, i, axis, -T_RANGE, T_RANGE, T_STEPS, sv, tv,
        ...
        self.q_sl  = {}
        self.q_txt = {}
        for i, (axis, val) in enumerate(zip(["qx", "qy", "qz", "qw"],
                                             [qx, qy, qz, qw])):
            sv = tk.DoubleVar(value=val)
    ...
    def _push_snapshot(self):
        """Save current state to history before a change is applied."""
    ...
    def _apply_snapshot(self, snap):
        ...
        try:
            for axis, val in snap["t"].items():
                self.t_sl[axis].set(val)
                self.t_txt[axis].set(f"{val:.5f}")
            for axis, val in snap["q"].items():
                self.q_sl[axis].set(val)
                self.q_txt[axis].set(f"{val:.8f}")
            for axis, val in snap["rpy"].items():
                self.rpy_sl[axis].set(val)
    ...
    # ------------------------------------------------------------------
    # Callbacks – translation (slider ↔ text, no cross-update needed)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Callbacks – quaternion slider/text → also update RPY
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Callbacks – RPY slider/text → also update quaternion
    # ------------------------------------------------------------------
    ...
    def _get_rpy_deg(self):
        return [self.rpy_sl[k].get() for k in ["roll", "pitch", "yaw"]]

    def _sync_rpy_from_quat(self):
        """Called while _updating=True – update RPY vars from current quat vars."""
    ...
    def _sync_quat_from_rpy(self):
        """Called while _updating=True – update quat vars from current RPY vars."""
        rpy = self._get_rpy_deg()
...
def publisher_thread(gui, tf_buffer, rate_hz=10):
    ...
    while not rospy.is_shutdown():
        if gui.dirty.is_set():
            ...
            pose = gui.get_pose()
            if pose is not None:
                corrected = compute_camera_link_pose(pose, tf_buffer)
                if corrected is not None:
                    broadcast_tf(corrected, broadcaster)
                    gui._set_status("TF published", "green")
                else:
                    broadcast_tf(pose, broadcaster)
                    gui._set_status("TF published (no rgb→base chain found)", "orange")
            else:
...
if __name__ == "__main__":
    rospy.init_node("xtion_tf_calibration_gui", anonymous=False)

    rospack = rospkg.RosPack()
    yaml_path = rospack.get_path("manipulation") + "/config/realsense_tf.yaml"

    tf_buffer = tf2_ros.Buffer()
    tf2_ros.TransformListener(tf_buffer)

    root = tk.Tk()
    gui = CalibrationGUI(root, yaml_path, tf_buffer)
